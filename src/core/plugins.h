...
public:
    ...
    virtual void render(const cerebra::BrainFrame& frame) = 0;
};
...
public:
    ...
    void emitEvent(EventType type, const cerebra::BrainFrame& frame) {
        if(type == EventType::Render) {
            for(auto& p : plugins) p->render(frame);
        }
    }
    void notifyRender(const cerebra::BrainFrame& frame) { emitEvent(EventType::Render, frame); }
};
