...
bool validateBrainActivityJSON(const std::string& json) {
    ...
    // Check for required keys in a more robust way
...
static bool g_multithreading_enabled = false;
...
std::vector<BrainFrame> compressData(const std::vector<BrainFrame>& data) {
    ...
    std::vector<BrainFrame> compressed;
    for (size_t i = 0; i < data.size(); ++i) {
        if (i % 2 == 0) compressed.push_back(data[i]);
    }
    return compressed; 
}
...
std::vector<BrainFrame> processStdinData() {
    ...
    return parseBrainActivityJSON(input);
}

void enableMultithreading() {
    g_multithreading_enabled = true;
}
...
void enablePGO() {
    // PGO logic would go here in a real build
}
