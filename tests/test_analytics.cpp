...
void t101() { BrainFrame f; f.regions={{"R1",0.9},{"R2",0.1}}; std::ostringstream oss; renderCorrelationMatrix(oss,f); ASSERT_TRUE(oss.str().find("Matrix")!=std::string::npos, "Corr fail"); }
...
void t106() { std::vector<BrainRegion> rs={{"R1",0.1},{"R2",0.8}}; performClustering(rs); ASSERT_TRUE(true, "Clust pass"); }
